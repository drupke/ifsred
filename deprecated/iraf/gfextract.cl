...
begin
    ...
    string tmpin, tmpslitdat,tmpslitvar,tmpslitdq
    ...
    real   detector_upper_spec_limit[3],dqthresh
    ...
    if (imgets.value == "0" && l_outimage!="") {
        gmosaic (tmpin, outimages=impaste, outpref="", fl_paste=no, 
            ...
            fl_real-, fl_vardq=l_fl_vardq, verbose=l_verbose)
        if (gmosaic.status != 0)
            goto error
        delpaste=yes
    } else {
    ...
    for (i=j1; i<=nslit; i+=1) {
        ...
        if (l_fl_vardq) {
            ...
            imcopy(impaste//"["//l_var_ext//",1]"//slitreg[i],
                   varsec,verbose-)
            gemhedit(varsec, "DISPAXIS", 1, "", delete-)
            imcopy(impaste//"["//l_dq_ext//",1]"//slitreg[i],
                   dqsec,verbose-)
        ...
        tmpslitdat = mktemp("tmpslitdat")

        apall (imgsec, tinfo.nrows, output=tmpslitdat, apertures="", \
        ...
        if (l_fl_vardq) {
            tmpslitvar = mktemp("tmpslitvar")
            tmpslitdq = mktemp("tmpslitdq")
            apall (varsec, tinfo.nrows, output=tmpslitvar, apertures="", \
                format="multispec", references=l_reference, profiles="", \
		inter=l_inter, find+, recenter=l_recenter, resize-, edit+, \
            	trace=l_trace, fittrace+, extract+, extras-, review-, \
            	line=l_line, nsum=l_nsum, lower=-2.5, upper=2.5, \
            	...
            	weights=l_weights, pfit="fit1d", clean-, saturation=INDEF, \
            	gain=1., lsigma=4., usigma=4., nsubaps=1)
            apall (dqsec, tinfo.nrows, output=tmpslitdq, apertures="", \
                format="multispec", references=l_reference, profiles="", \
		inter=l_inter, find+, recenter=l_recenter, resize-, edit+, \
            	trace=l_trace, fittrace+, extract+, extras-, review-, \
            	line=l_line, nsum=l_nsum, lower=-2.5, upper=2.5, \
        ...
        # kluge the header
        gemhedit(tmpslitdat//".fits", "CDELT*", "", "", delete+)
        gemhedit(tmpslitdat//".fits", "CD1_1", 1.0, "", delete-)
        gemhedit(tmpslitdat//".fits", "CD2_2", 1.0, "", delete-)
        gemhedit(tmpslitdat//".fits", "CRPIX1", 1, "", delete-)
        gemhedit(tmpslitdat//".fits", "CRPIX2", 1, "", delete-)
        gemhedit(tmpslitdat//".fits", "CRVAL1", 1., "", delete-)
        gemhedit(tmpslitdat//".fits", "CRVAL2", 1., "", delete-)

        if (l_fl_vardq) {
          gemhedit(tmpslitvar//".fits", "CDELT*", "", "", delete+)
          gemhedit(tmpslitvar//".fits", "CD1_1", 1.0, "", delete-)
          gemhedit(tmpslitvar//".fits", "CD2_2", 1.0, "", delete-)
          gemhedit(tmpslitvar//".fits", "CRPIX1", 1, "", delete-)
          gemhedit(tmpslitvar//".fits", "CRPIX2", 1, "", delete-)
          gemhedit(tmpslitvar//".fits", "CRVAL1", 1., "", delete-)
          gemhedit(tmpslitvar//".fits", "CRVAL2", 1., "", delete-)
          gemhedit(tmpslitdq//".fits", "CDELT*", "", "", delete+)
          gemhedit(tmpslitdq//".fits", "CD1_1", 1.0, "", delete-)
          gemhedit(tmpslitdq//".fits", "CD2_2", 1.0, "", delete-)
          gemhedit(tmpslitdq//".fits", "CRPIX1", 1, "", delete-)
          gemhedit(tmpslitdq//".fits", "CRPIX2", 1, "", delete-)
          gemhedit(tmpslitdq//".fits", "CRVAL1", 1., "", delete-)
          gemhedit(tmpslitdq//".fits", "CRVAL2", 1., "", delete-)
        }

        printlog("Extraction done", l_logfile, l_verbose)

        # Separate the multispec files
        imcopy(tmpslitdat,sci,verb-)
        if (l_fl_vardq) {
            imcopy(tmpslitvar,var,verb-)
            imcopy(tmpslitdq,dq,verb-)
        }
        ...
        if (l_fl_vardq) {
            ...
            imdelete(tmpslitvar//","//tmpslitdq, verify-, >& "dev$null")
        }
        imdelete(imgsec//","//sci//","//var//","//dq, verify-, >& "dev$null")
        imdelete(tmpslitdat, verify-, >& "dev$null")
